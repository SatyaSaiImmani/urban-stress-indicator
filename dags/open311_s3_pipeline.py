...
# Local Airflow machine paths
LOCAL_ENV_FILE = "/D/sxi219/env.sh"

# Remote SSH connection id already created in Airflow
SSH_CONN_ID = "spark_remote_ssh"

# Remote target path for copied env file
REMOTE_ENV_FILE = "/root/urban-stress-indicator/env.sh"
...
with DAG(
    dag_id=DAG_ID,
    description="Open311 raw->bronze->silver locally, copy env to remote, aggregate->baseline remotely via SSH",
    ...
    def run_silver_local(run_context: dict) -> None:
        ...
        bash scripts/open311/run_silver.sh \
          ...
          --ingest-date "{run_context['ingest_date']}" \
          --batch-id "{run_context['batch_id']}" \
          --mapping-file "$LOCAL_REPO_ROOT/etl/open311/configs/open311_category_mapping.csv" \
          --env-file "$LOCAL_ENV_FILE" \
          --python-bin "$LOCAL_PYTHON_BIN"
        """
        _run_local_bash(cmd)
    ...
    run_aggregate_remote = SSHOperator(
        ...
        cd "$REMOTE_REPO_ROOT"
        bash scripts/open311/run_aggregates.sh \
          --bucket "$URBAN_STRESS_BUCKET" \
